...
def main(args):
	...
	while(True):
		...
		if(opcode.getResult()=="PUSHAD"):
			imm.run(curAddr)
			imm.stepOver()
			Addr=imm.getRegs()["ESP"]
			imm.setHardwareBreakpoint(Addr,type=2,size=4)
			imm.run()
			curAddr=imm.getRegs()["EIP"]
			imm.log("curAddr: 0x%08X" % curAddr)
			#return
			while True:
				op2=imm.disasm(curAddr)
				opsize=op2.getOpSize()
				if op2.getResult()=="CALL EAX":
					imm.stepIn()
			...
			if opcode.getResult()=="JMP EAX":
				...
				imm.log("curAddr: 0x%08X" % curAddr)
				imm.setHardwareBreakpoint(jmpAddr)
				jmp_eax=True
				#imm.run(jmpAddr)
				#break
			else:
